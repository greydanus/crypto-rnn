...
def accuracy(model, dataloader):
    ...
    kl = dataloader.key_len
    for _ in range(trials):
        batch = dataloader.next_batch(model.batch_size)
        y = np.stack(batch[1])
        y_hat = model.decode(batch[0])
        mean_acc += 1./trials * np.sum(y[:,kl:,:]*y_hat[:,kl:,:])/(y.shape[0]*(y.shape[1]-kl)) # TODO: make this less awk
    return mean_acc*100

# qualitative eval
def sample(model, dataloader):
    model.reset_states()
    plaintext = u'YOUKNOWNOTHINGJONSNOW'
    key = dataloader.rands(3).decode('unicode-escape')
    ciphertext = key + dataloader.encode(key, plaintext)
    decoded = ''
